...
pub fn main() {
    ...
    let control_background = Rect::new(998, 0, 1000, 1000);
    ...
    let start_board_button: Box<dyn Button> = Box::new(StandardButton {
        height: 100,
        width: 200,
        location: Point::new(window_width as i32 - 200, 600),
        text_color: BLACK,
    ...
    let piece_select: Box<dyn Button> = Box::new(OptionButton::new(
        50,
        200,
        Point::new(window_width as i32 - 200, 300),
    ...
    let upload_map_button: Box<dyn Button> = Box::new(StandardButton {
        height: 50,
        width: 200,
        location: Point::new(window_width as i32 - 200, 100),
        text_color: WHITE,
        background_color: PRIMARY_COLOR,
        hover_color: HOVER_COLOR,
    ...
    let save_map_button: Box<dyn Button> = Box::new(StandardButton {
        height: 50,
        width: 200,
        location: Point::new(window_width as i32 - 200, 200),
        text_color: WHITE,
        background_color: PRIMARY_COLOR,
        hover_color: HOVER_COLOR,
    ...
    let mut board_control_components: HashMap<&'static str, Box<dyn Button>> = HashMap::from([
        ("upload", upload_map_button),
        ("save", save_map_button),
        ("start", start_board_button),
        ("select_piece", piece_select),
    ]);
    ...
    'running: loop {
        ...
        if (window_width, window_height) != current_size {
            ...
            game_board.draw(&mut canvas);
            board_control_components.iter_mut().for_each(|(_, a)| {
                a.change_location(Point::new(
                    window_width as i32 - a.get_width() as i32,
                    a.get_location().y(),
                ))
            })
        }
        ...
        if select_file {
            if !components_drawn || util::mouse_over(control_background, mouse_position) {
                /*------- File Selection Menu -------*/
                board_control_components
                    .iter_mut()
                    .for_each(|(_, a)| a.change_active(false));
                    ...
                    if search_text_length == 0 {
                        directory_buttons
                            ...
                            .for_each(|a| {
                                ...
                                a.draw(&mut canvas, &texture_creator, Some(mouse_position));
                            });
                    }
                    if search_file.text.trim().len() != search_text_length {
                        ...
                        directory_buttons
                            ...
                            .for_each(|a| {
                                ...
                                count += a.active_options_count();
                                a.draw(&mut canvas, &texture_creator, Some(mouse_position))
                            });
                        go_back_button.draw(&mut canvas, &texture_creator, Some(mouse_position));
                ...
                canvas.set_draw_color(Color::RGB(87, 87, 81));
                canvas.fill_rect(control_background).unwrap();
                search_file.draw(&mut canvas, &texture_creator, &mut font);
                directory_buttons
                    .iter_mut()
            ...
            if components_drawn {
                if util::mouse_over(control_background, mouse_position) {
                    board_control_components.iter_mut().for_each(|(_, a)| {
                        a.draw(
                            &mut canvas,
                            &texture_creator,
                ...
                board_control_components
                    .iter_mut()
                    .for_each(|(_, a)| a.change_active(true));
        ...
        if mouse_state.left() {
            if game_board.on_click(mouse_position).0 {
                ...
                game_board.draw(&mut canvas);
            } else if util::mouse_over(control_background, mouse_position) {
                if !select_file {
                    match board_control_components
                        .iter_mut()
                        .find(|(_, b)| b.mouse_over_component(mouse_position))
                    {
                        Some((name, b)) => match *name {
                            "start" => {
                            ...
                            "select_piece" => match b.on_click(mouse_position).1 {
