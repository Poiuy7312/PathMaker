use sdl2::mouse::MouseState;
use sdl2::pixels::{Color, PixelFormatEnum};
use sdl2::rect::{Point, Rect};
use sdl2::render::{Canvas, Texture, TextureCreator};
use sdl2::surface::Surface;
use sdl2::video::{Window, WindowContext};

use sdl2::ttf;

#[derive(Clone)]
pub struct Button {
    pub height: u32,
    pub width: u32,
    pub location: Point,
    pub text_color: [u8; 3],
    pub background_color: [u8; 3],
    pub hover_color: [u8; 3],
    pub text: String,
    pub hover: bool,
    pub active: bool,
}

impl Button {
    pub fn mouse_over_component(&self, mouse_position: (i32, i32)) -> bool {
        let component: Rect = self.get_rect(self.location);
        let mouse_point = sdl2::rect::Point::new(mouse_position.0, mouse_position.1);
        return component.contains_point(mouse_point);
    }
    pub fn on_click(&self, mouse_state: &MouseState) -> bool {
        if self.mouse_over_component((mouse_state.x(), mouse_state.y())) && self.active {
            return true;
        }
        return false;
    }
    pub fn change_mode(&mut self, mode: bool) {
        self.hover = mode;
    }
    fn get_rect(&self, point: Point) -> Rect {
        Rect::from_center(point, self.width, self.height)
    }
    pub fn draw<'a>(
        &self,
        canvas: &mut Canvas<Window>,
        texture_creator: &'a TextureCreator<WindowContext>,
        mouse_state: Option<&MouseState>,
    ) {
        let button_background: Rect = self.get_rect(self.location);
        let font_size = 8 * self.text.chars().count() as u32;
        let button_outline = Rect::from_center(self.location, self.width + 5, self.height + 5);
        let text_map = Rect::from_center(self.location, font_size, 20);
        let mouse_state = mouse_state.expect("No mouse state given");
        let ttf_context: ttf::Sdl2TtfContext = ttf::init().unwrap();

        let file = ttf_context.load_font("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 124);

        match file {
            Ok(value) => {
                let mut font = value;
                font.set_style(sdl2::ttf::FontStyle::BOLD);
                canvas.set_draw_color(Color::RGB(0, 0, 0));
                canvas.fill_rect(button_outline).unwrap();

                // render a surface, and convert it to a texture bound to the canvas
                if self.mouse_over_component((mouse_state.x(), mouse_state.y())) {
                    canvas.set_draw_color(Color::RGB(
                        self.hover_color[0],
                        self.hover_color[1],
                        self.hover_color[2],
                    ));
                    canvas.fill_rect(button_background).unwrap();
                } else {
                    canvas.set_draw_color(Color::RGB(
                        self.background_color[0],
                        self.background_color[1],
                        self.background_color[2],
                    ));
                    canvas.fill_rect(button_background).unwrap();
                }

                let font_surface = font
                    .render(&self.text)
                    .blended(Color::RGB(
                        self.text_color[0],
                        self.text_color[1],
                        self.text_color[2],
                    ))
                    .map_err(|e| e.to_string())
                    .unwrap();

                let font_texture: Texture<'_> = texture_creator
                    .create_texture_from_surface(&font_surface)
                    .map_err(|e| e.to_string())
                    .unwrap();
                canvas
                    .copy(&font_texture, None, text_map)
                    .expect("Button unable to display text");
            }
            Err(e) => {
                println!("{}", e);
            }
        }
    }
}

impl PartialEq for Button {
    fn eq(&self, other: &Self) -> bool {
        self.location == other.location
    }
}
